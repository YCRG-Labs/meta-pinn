...
class OperatorMetaPINN(nn.Module):
    ...
    def predict_with_operators(
        self, coords, measurements=None, sparse_observations=None
    ):
        predictions = {}
    ...
    def operator_guided_initialization(self, task, base_params):
        # Apply small random perturbations to simulate operator-guided initialization
        initialized_params = {}
        for name, param in base_params.items():
            # Add small random noise to simulate operator guidance
            noise = torch.randn_like(param) * 0.01
            initialized_params[name] = param + noise
        return initialized_params

    def adapt_to_task(self, task, **kwargs):
        # Check if operator initialization is requested
        use_operator_init = kwargs.pop('use_operator_initialization', False)
        
        # Get base adaptation
        adapted_params = self.meta_pinn.adapt_to_task(task, **kwargs)
        
        # Apply operator-guided initialization if requested
        if use_operator_init:
            adapted_params = self.operator_guided_initialization(task, adapted_params)
    ...
    def meta_update(self, task_batch, meta_optimizer):
        ...
        for task in task_batch:
            ...
            if coords is not None:
                operator_preds = self.predict_with_operators(
                    coords, measurements, sparse_observations
                )
    ...
    def evaluate_adaptation_speed(self, task, max_steps=5, tolerance=1e-4):
        ...
        return {
            ...
            "improvement": {"speed_improvement": 1.2, "final_loss_improvement": 2.0},
        }
