...
class InverseFourierNeuralOperator(nn.Module):
    ...
    def grid_to_tensor(self, sparse_observations: Dict[str, torch.Tensor]) -> torch.Tensor:
        """
        ...
        grid_tensor[:, 0, :, :] = grid_coords[:, :, 0].unsqueeze(0).expand(batch_size, -1, -1)
        grid_tensor[:, 1, :, :] = grid_coords[:, :, 1].unsqueeze(0).expand(batch_size, -1, -1)
        ...
        for b in range(batch_size):
            for i, coord in enumerate(coords[b]):
                ...
                if len(values.shape) == 3 and values.shape[2] > 0:
                    grid_tensor[b, 2:2+values.shape[2], y_idx, x_idx] = values[b, i, :]
