...
class UncertaintyCalibrator:
    ...
    def fit(self, uncertainties: torch.Tensor, errors: torch.Tensor, 
            weights: Optional[torch.Tensor] = None) -> 'UncertaintyCalibrator':
        """
    ...
    def evaluate_calibration(self, uncertainties: torch.Tensor, errors: torch.Tensor,
    ...
    def plot_reliability_diagram(self, uncertainties: torch.Tensor, errors: torch.Tensor,
        ...
        ax1.plot([0, max_val], [0, max_val], 'k--', alpha=0.7, label='Perfect calibration')
        
        # Plot actual calibration
        ax1.scatter(bin_confidences, bin_accuracies, s=bin_counts/np.max(bin_counts)*200,
                   alpha=0.7, c='blue', label='Observed')
        ...
        ax2.hist(uncertainties_np, bins=self.n_bins, alpha=0.7, color='blue', edgecolor='black')
    ...
    def physics_informed_calibration(self, model, coords: torch.Tensor, 
        ...
        physics_losses = model.physics_loss(coords, task_info, params, create_graph=False)
    ...
    def adaptive_calibration(self, uncertainties: torch.Tensor, errors: torch.Tensor,
                           physics_residuals: torch.Tensor, 
                           physics_weight: float = 0.5) -> torch.Tensor:
        """
        ...
        physics_residuals = torch.where(torch.isfinite(physics_residuals), 
                                      physics_residuals, 
        ...
        physics_uncertainty = torch.where(torch.isfinite(physics_uncertainty),
                                        physics_uncertainty,
                                        torch.tensor(1e-3, device=physics_uncertainty.device))
        
        # Combine uncertainties
        combined_uncertainty = (
            (1 - physics_weight) * calibrated_uncertainties + 
            physics_weight * physics_uncertainty
        )
        
        # Final safety check
        combined_uncertainty = torch.clamp(combined_uncertainty, min=1e-8)
        combined_uncertainty = torch.where(torch.isfinite(combined_uncertainty),
                                         combined_uncertainty,
    ...
    def get_calibration_summary(self) -> Dict[str, Any]:
        ...
        if self.reliability_data is not None:
            summary.update({
                ...
                'mean_bin_accuracy': float(np.mean(self.reliability_data['bin_accuracies'])),
                'mean_bin_confidence': float(np.mean(self.reliability_data['bin_confidences'])),
...
class CalibrationEvaluator:
    ...
    def evaluate_model_calibration(self, model, test_tasks: List[Dict], 
                                 calibrator: Optional[UncertaintyCalibrator] = None,
                                 n_mc_samples: int = 100) -> Dict[str, Any]:
        """
        ...
        for task in test_tasks:
            ...
            with torch.no_grad():
                ...
                # Compute physics residuals
                physics_losses = model.physics_loss(query_coords, task_info, create_graph=False)
        ...
        physics_metrics = calibrator.evaluate_calibration(physics_uncertainty, all_errors)
    ...
    def compare_calibration_methods(self, uncertainties: torch.Tensor, errors: torch.Tensor,
                                  methods: List[str] = ['isotonic']) -> Dict[str, Dict]:
        """
