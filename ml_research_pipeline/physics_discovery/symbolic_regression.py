...
class ExpressionGenerator:
    ...
    def __init__(
        ...
        if operators is None:
            # Focus on physics-relevant operators
            self.operators = ["+", "-", "*", "/", "**", "sqrt", "log"]
        else:
            self.operators = operators

        # Define operator arities (simplified for better discovery)
        self.binary_ops = ["+", "-", "*", "/", "**"]
        self.unary_ops = ["sqrt", "log", "abs"]

        # Physics-relevant constants
        self.constants = [0.1, 0.5, 1.0, 2.0, -0.5, -1.0]

    def generate_random_expression(self, depth: int = 0) -> sp.Expr:
        """
        Generate a random symbolic expression focused on physics relationships.
        ...
        if depth >= self.max_depth or (depth > 0 and random.random() < 0.4):
            ...
            if random.random() < 0.8:  # Favor variables over constants
                return sp.Symbol(random.choice(self.variables))
            else:
                return sp.Float(random.choice(self.constants))

        # Non-terminal node: operator
        if random.random() < 0.85:  # Favor binary operators for physics relationships
            ...
            if op == "+":
                ...
                # Safer division with larger epsilon
                return left / (right + sp.Float(0.01))
            elif op == "**":
                # Simple power relationships common in physics
                if random.random() < 0.5:
                    # Use simple integer powers
                    power = random.choice([0.5, 1, 2, -1, -0.5])
                    return left ** sp.Float(power)
                else:
                    # Use variable power but limit it
                    power = sp.Min(sp.Abs(right), 2)
                    return left ** power
        else:  # Unary operator
            ...
            if op == "sqrt":
                return sp.sqrt(sp.Abs(operand) + sp.Float(0.01))
            elif op == "log":
                # Natural log with safety
                return sp.log(sp.Abs(operand) + sp.Float(0.1))
            elif op == "abs":
    ...
    def crossover_expressions(
        self, expr1: sp.Expr, expr2: sp.Expr
...
class NeuralSymbolicRegression(nn.Module):
    ...
    def __init__(
        ...
        # Initialize expression generator with simpler operators
        self.expr_generator = ExpressionGenerator(
            variables=variables, 
            max_depth=min(3, max_expression_depth),  # Force simpler expressions
            operators=["+", "-", "*", "/", "**"]  # Only basic operators
        )
    ...
    def generate_expression_candidates(self, n_candidates: int = None) -> List[sp.Expr]:
        """
        Generate initial population focusing on simple, discoverable expressions.
        ...
        # Generate simple expressions first (more likely to succeed)
        simple_candidates = self._generate_simple_candidates()
        candidates.extend(simple_candidates)
        
        # Fill remaining with random expressions if needed
    ...
    def _generate_simple_candidates(self) -> List[sp.Expr]:
        """Generate simple, physics-relevant expressions."""
        candidates = []
        
        # Single variables
        for var in self.variables:
            candidates.append(sp.Symbol(var))
        
        # Simple linear combinations (a*x + b*y)
        if len(self.variables) >= 2:
            for i in range(len(self.variables)):
                for j in range(i+1, len(self.variables)):
                    var1, var2 = self.variables[i], self.variables[j]
                    candidates.append(sp.Symbol(var1) + sp.Symbol(var2))
                    candidates.append(2*sp.Symbol(var1) + sp.Symbol(var2))
                    candidates.append(sp.Symbol(var1) + 2*sp.Symbol(var2))
                    # Add more coefficient variations
                    candidates.append(0.5*sp.Symbol(var1) + sp.Symbol(var2))
                    candidates.append(sp.Symbol(var1) + 0.5*sp.Symbol(var2))
                    candidates.append(0.1*sp.Symbol(var1) + sp.Symbol(var2))
                    candidates.append(sp.Symbol(var1) + 0.1*sp.Symbol(var2))
        
        # Simple power laws (common in physics)
        for var in self.variables:
            candidates.append(sp.Symbol(var) ** 2)
            candidates.append(sp.Symbol(var) ** 0.5)
            candidates.append(1 / sp.Symbol(var))
    ...
    def _try_linear_regression(self, flow_data: Dict[str, np.ndarray], target: np.ndarray) -> Optional[SymbolicExpression]:
        """Try linear regression to find the best linear combination."""
        try:
            from sklearn.linear_model import LinearRegression
            
            # Prepare data
            var_names = list(flow_data.keys())
            X = np.column_stack([flow_data[var] for var in var_names])
            
            # Fit linear regression
            reg = LinearRegression().fit(X, target)
            pred = reg.predict(X)
            r2 = sklearn_r2_score(target, pred)
            
            if r2 > 0.5:  # Only return if reasonably good
                # Create symbolic expression
                terms = []
                for i, coef in enumerate(reg.coef_):
                    if abs(coef) > 1e-6:  # Only include significant coefficients
                        if abs(coef - 1.0) < 0.1:
                            terms.append(sp.Symbol(var_names[i]))
                        elif abs(coef + 1.0) < 0.1:
                            terms.append(-sp.Symbol(var_names[i]))
                        else:
                            terms.append(float(coef) * sp.Symbol(var_names[i]))
                
                if abs(reg.intercept_) > 1e-6:
                    terms.append(float(reg.intercept_))
                
                if terms:
                    expr = sum(terms) if len(terms) > 1 else terms[0]
                    
                    # Create SymbolicExpression object
                    fitness, r2_check, mse = self.evaluate_expression_fitness(expr, flow_data, target)
                    ...
                    return SymbolicExpression(
                        ...
                        r2_score=r2_check,
                        mse=mse,
                        variables=list(expr.free_symbols) if hasattr(expr, 'free_symbols') else []
                    )
    ...
    def evaluate_expression_fitness(
        ...
        """
        try:
            # Convert expression to numerical function with safe evaluation
            ...
            # Replace invalid values with target mean to avoid NaN propagation
            if not np.isfinite(predictions).all():
                target_mean = np.mean(target)
                predictions = np.where(np.isfinite(predictions), predictions, target_mean)
            
            # Clip extreme values to prevent numerical instability
            target_std = np.std(target)
            target_mean = np.mean(target)
            predictions = np.clip(predictions, 
                                target_mean - 5 * target_std, 
                                target_mean + 5 * target_std)

            # Compute metrics with additional safety
            try:
                mse = mean_squared_error(target, predictions)
                r2 = sklearn_r2_score(target, predictions)
            ...
            fitness = max(-10.0, r2 - complexity_penalty - mse * 0.1)

            return fitness, r2, max(0.0, mse)
    ...
    def discover_viscosity_law(
        ...
        """
        # Try simple linear regression first (most likely to succeed)
        best_expr = self._try_linear_regression(flow_data, viscosity_data)
        if best_expr and best_expr.r2_score > 0.8:
            return best_expr
        ...
        for generation in range(self.max_generations):
            ...
            # Sort by R² score first (prioritize predictive power), then by fitness
            evaluated_population.sort(key=lambda x: (x.r2_score, x.fitness), reverse=True)
            ...
            if validation_score >= validation_threshold:
                print(
                    f"Validation threshold {validation_threshold} achieved at generation {generation}"
                )
                break
    ...
    def _create_next_generation(
        self, evaluated_population: List[SymbolicExpression]
    ) -> List[sp.Expr]:
        ...
        while len(next_generation) < self.population_size:
            ...
            if random.random() < self.mutation_rate:
                child1 = self.expr_generator.mutate_expression(
                    child1, self.mutation_rate
                )
            if random.random() < self.mutation_rate:
                child2 = self.expr_generator.mutate_expression(
                    child2, self.mutation_rate
                )
    ...
    def _tournament_selection(
        self, population: List[SymbolicExpression], tournament_size: int = 3
    ) -> SymbolicExpression:
    ...
    def validate_discovered_physics(
        self,
        ...
        try:
            ...
            input_values = [
                validation_data[var] for var in self.variables if var in validation_data
            ]
            ...
            if len(input_values) > 0:
                ...
                if target_std > 1e-8:
                    physics_consistency = 1.0 - min(
                        1.0, np.std(predictions) / target_std
                    )
                else:
    ...
    def generate_physics_hypothesis(
        self,
        discovered_expr: SymbolicExpression,
        ...
        hypothesis = (
            f"The discovered physics relationship suggests that the target quantity "
        )
        hypothesis += f"depends on the input variables according to the expression: {expr_str}.\n\n"

        # Analyze expression components
        variables_used = [
            str(var) for var in expr.free_symbols if str(var) in self.variables
        ]
        ...
        if discovered_expr.r2_score >= 0.9:
            ...
            hypothesis += (
                f"The expression shows good fit (R² = {discovered_expr.r2_score:.3f}), "
            )
            ...
            hypothesis += (
                "suggesting either noise in data or missing physical effects.\n"
            )

        return hypothesis

    def export_best_expression(
        self, format: str = "latex", simplify: bool = True
    ) -> str:
        """
