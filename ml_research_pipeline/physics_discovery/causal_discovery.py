...
class PhysicsCausalDiscovery:
    ...
    def __init__(
        ...
        self.viscosity_variables = [
            "viscosity",
            "viscosity_gradient_x",
            "viscosity_gradient_y",
            "viscosity_type",
            "viscosity_params",
        ]

    def discover_viscosity_dependencies(
        self, flow_data: Dict[str, np.ndarray]
        ...
        for flow_var in self.flow_variables:
            ...
            for visc_var in self.viscosity_variables:
                ...
                mi_score, p_value, ci = self._compute_mutual_information(
                    prepared_data[flow_var], prepared_data[visc_var]
                )

                # Check significance and minimum threshold
                if (
                    p_value < self.significance_threshold
                    and mi_score > self.min_mutual_info
                ):
    ...
    def generate_physics_hypothesis(
        self, relationships: List[CausalRelationship], graph: nx.DiGraph
    ) -> str:
        """
        ...
        influenced_vars = sorted(in_degrees.items(), key=lambda x: x[1], reverse=True)[
            :3
        ]
        ...
        if influenced_vars and influenced_vars[0][1] > 0:
            hypothesis_parts.append(
                f"Most influenced variable: {influenced_vars[0][0]}"
            )
    ...
    def visualize_causal_graph(
        ...
        nx.draw_networkx_nodes(
            graph, pos, node_color=node_colors, node_size=1000, alpha=0.8
        )
        ...
        if edge_weights:
            ...
            nx.draw_networkx_edges(
                graph, pos, width=edge_widths, alpha=0.6, edge_color="gray"
            )
        ...
        plt.title(
            "Causal Relationships in Physics Variables", fontsize=14, fontweight="bold"
        )
        plt.axis("off")
        plt.tight_layout()
        ...
        # plt.show()  # Disabled to prevent pop-ups
