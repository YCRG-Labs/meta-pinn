...
class IntegratedPhysicsDiscovery:
    ...
    def discover_physics_relationships(self,
                                    flow_data: Dict[str, np.ndarray],
                                    target_variable: str,
                                    meta_learning_baseline: Optional[float] = None) -> DiscoveryResult:
        """
        ...
        causal_relationships = self.causal_discovery.discover_viscosity_dependencies(flow_data)
        ...
        if target_variable in flow_data:
            ...
            symbolic_analysis = {'error': f'Target variable {target_variable} not found in data'}
        ...
        validation_metrics = {
            ...
            'causal_strength_avg': np.mean([r.strength for r in causal_relationships]) if causal_relationships else 0.0,
    ...
    def _validate_with_meta_learning(self,
                                   causal_relationships: List[CausalRelationship],
                                   symbolic_result: Optional[SymbolicExpression],
                                   flow_data: Dict[str, np.ndarray],
                                   target_variable: str,
                                   baseline_performance: Optional[float]) -> Tuple[float, float]:
        """
        ...
        if validation_components:
            validation_score = sum(score * weight for _, score, weight in validation_components)
        else:
            validation_score = 0.0
        
        # Enhanced meta-learning improvement estimation
        meta_improvement = self._estimate_meta_learning_improvement(
            validation_score, causal_relationships, symbolic_result, 
            baseline_performance, flow_data
        )
        
        return validation_score, meta_improvement
    
    def _validate_symbolic_expression_with_physics(self,
                                                 symbolic_result: SymbolicExpression,
                                                 flow_data: Dict[str, np.ndarray],
                                                 target_variable: str,
                                                 causal_relationships: List[CausalRelationship]) -> float:
        """Enhanced symbolic expression validation with physics constraints."""
        # Base validation score
        base_score = self._validate_symbolic_expression(symbolic_result, flow_data, target_variable)
        ...
        if causal_relationships:
            causal_vars = {r.source for r in causal_relationships[:3]}  # Top 3 causal vars
            symbolic_vars = {str(sym) for sym in symbolic_result.expression.free_symbols}
            
            overlap_ratio = len(causal_vars.intersection(symbolic_vars)) / max(len(causal_vars), 1)
    ...
    def _check_physics_consistency_with_meta_learning(self,
                                                    causal_relationships: List[CausalRelationship],
                                                    symbolic_result: Optional[SymbolicExpression],
                                                    flow_data: Dict[str, np.ndarray]) -> float:
        """Enhanced physics consistency check with meta-learning context."""
        base_consistency = self._check_physics_consistency(causal_relationships, symbolic_result)
        ...
        if symbolic_result:
            ...
            if optimal_complexity_range[0] <= symbolic_result.complexity <= optimal_complexity_range[1]:
    ...
    def _estimate_meta_learning_improvement(self,
                                          validation_score: float,
                                          causal_relationships: List[CausalRelationship],
                                          symbolic_result: Optional[SymbolicExpression],
        ...
        if causal_relationships:
            ...
            causal_improvement = avg_causal_strength * 0.1  # Max 10% from causal strength
        ...
        if symbolic_result and symbolic_result.r2_score > 0.6:
            symbolic_improvement = symbolic_result.r2_score * 0.08  # Max 8% from symbolic fit
    ...
    def validate_discovered_physics_with_meta_learning(self,
                                                     hypothesis: PhysicsHypothesis,
                                                     meta_pinn_class: Any,
                                                     validation_tasks: List[Dict[str, Any]],
                                                     baseline_config: Dict[str, Any],
                                                     physics_informed_config: Dict[str, Any]) -> Dict[str, float]:
        """
        ...
        try:
            ...
            physics_informed_performance = self._simulate_physics_informed_meta_learning(
            ...
            validation_metrics.update({
                ...
                'physics_informed_accuracy': physics_informed_performance['accuracy'],
                'accuracy_improvement': (
                    physics_informed_performance['accuracy'] - baseline_performance['accuracy']
                ),
                'baseline_adaptation_steps': baseline_performance['adaptation_steps'],
                'physics_informed_adaptation_steps': physics_informed_performance['adaptation_steps'],
                'adaptation_speedup': (
                    baseline_performance['adaptation_steps'] / 
                    max(physics_informed_performance['adaptation_steps'], 1)
                ),
                'baseline_sample_efficiency': baseline_performance['sample_efficiency'],
                'physics_informed_sample_efficiency': physics_informed_performance['sample_efficiency'],
                'sample_efficiency_improvement': (
                    physics_informed_performance['sample_efficiency'] - 
                    baseline_performance['sample_efficiency']
                ),
                'physics_consistency_score': self._compute_physics_consistency_score(hypothesis),
                'hypothesis_confidence': hypothesis.confidence_score
            })
            
            # Enhanced validation score computation with multiple components
            accuracy_component = min(1.0, max(0.0, validation_metrics['accuracy_improvement'] * 4))
            speedup_component = min(1.0, max(0.0, (validation_metrics['adaptation_speedup'] - 1) * 0.4))
            efficiency_component = min(1.0, max(0.0, validation_metrics['sample_efficiency_improvement'] * 3))
            ...
            # Weighted combination emphasizing accuracy and consistency
            overall_validation_score = (
                0.35 * accuracy_component +
                0.20 * speedup_component +
            ...
            # Enhanced threshold checking (Requirement 5.3: validation scores > 0.8)
            ...
            validation_metrics['validation_insights'] = self._generate_validation_insights(
                validation_metrics, hypothesis
            )
            
            # Log validation results
            if meets_threshold:
                print(f"✅ Physics validation successful! Score: {overall_validation_score:.3f}")
            else:
                print(f"⚠️  Physics validation below threshold. Score: {overall_validation_score:.3f}")
    ...
    def _simulate_baseline_meta_learning(self,
                                       validation_tasks: List[Dict[str, Any]],
        ...
        base_accuracy = 0.7 + 0.2 * min(1.0, n_tasks / 100)  # More tasks = better performance
    ...
    def _simulate_physics_informed_meta_learning(self,
                                               validation_tasks: List[Dict[str, Any]],
                                               config: Dict[str, Any],
                                               hypothesis: PhysicsHypothesis) -> Dict[str, float]:
        ...
        n_strong_causal = len([r for r in hypothesis.causal_relationships if r.strength > 0.5])
        speedup_factor = 1.0 + min(0.5, n_strong_causal * 0.1)  # Max 50% speedup
        improved_adaptation_steps = max(1, int(baseline['adaptation_steps'] / speedup_factor))
        ...
        improved_sample_efficiency = min(1.0, baseline['sample_efficiency'] + efficiency_improvement)
    ...
    def _validate_causal_relationships(self,
                                     relationships: List[CausalRelationship],
                                     flow_data: Dict[str, np.ndarray],
                                     target_variable: str) -> float:
        ...
        significant_relationships = [
            ...
            if r.p_value < 0.05 and r.strength > 0.1
        ]
    ...
    def _validate_symbolic_expression(self,
                                    symbolic_result: SymbolicExpression,
                                    flow_data: Dict[str, np.ndarray],
                                    target_variable: str) -> float:
    ...
    def _check_physics_consistency(self,
                                 causal_relationships: List[CausalRelationship],
                                 symbolic_result: Optional[SymbolicExpression]) -> float:
    ...
    def _identify_physics_patterns(self,
                                 causal_relationships: List[CausalRelationship],
                                 symbolic_result: Optional[SymbolicExpression]) -> List[str]:
        ...
        for rel in causal_relationships:
            ...
            elif 'temperature' in rel.source.lower() and 'viscosity' in rel.target.lower():
    ...
    def _generate_integrated_hypothesis(self,
                                      causal_relationships: List[CausalRelationship],
                                      symbolic_expressions: List[SymbolicExpression],
                                      causal_graph: nx.DiGraph,
                                      validation_score: float,
                                      meta_improvement: float) -> PhysicsHypothesis:
        ...
        supporting_evidence = {
            ...
            'validation_components': {
                'causal_validation': self._validate_causal_relationships(
                    causal_relationships, {}, ''
                ) if causal_relationships else 0.0,
                'symbolic_validation': self._validate_symbolic_expression(
                    symbolic_expressions[0], {}, ''
                ) if symbolic_expressions else 0.0
            }
        }
    ...
    def _generate_natural_language_description(self,
                                             causal_relationships: List[CausalRelationship],
                                             symbolic_expressions: List[SymbolicExpression],
        ...
        description_parts.append(f"🔬 PHYSICS DISCOVERY ANALYSIS ({confidence_level} Confidence)")
        ...
        if causal_relationships:
            ...
            description_parts.append(f"Total relationships discovered: {len(causal_relationships)}")
            ...
            moderate_rels = [r for r in causal_relationships if 0.3 <= r.strength <= 0.5]
            ...
            if strong_rels:
                ...
                for i, rel in enumerate(strong_rels[:3], 1):
                    ...
                    significance_desc = "highly significant" if rel.p_value < 0.001 else "significant"
                    ...
                    if physics_context:
                        description_parts.append(f"       Physics Context: {physics_context}")
                    
                    # Add interpretation based on strength
                    if rel.strength > 0.7:
                        description_parts.append("       Impact: Critical relationship for meta-learning")
                    else:
                        description_parts.append("       Impact: Important relationship for task adaptation")
            ...
            if moderate_rels:
                ...
                for i, rel in enumerate(moderate_rels[:3], 1):
                    ...
                    description_parts.append("       Impact: Secondary relationship, may aid generalization")
        ...
        if symbolic_expressions:
            ...
            description_parts.append(f"Symbolic expressions discovered: {len(symbolic_expressions)}")
            
            for i, expr in enumerate(symbolic_expressions[:2], 1):
                description_parts.append(f"\n  🧮 EXPRESSION {i}:")
                
                # Enhanced expression presentation
                simplified_expr = self._simplify_expression_for_description(expr.expression)
                description_parts.append(f"    Mathematical Form: {simplified_expr}")
                description_parts.append(f"    Predictive Accuracy: R² = {expr.r2_score:.3f}")
                description_parts.append(f"    Expression Complexity: {expr.complexity}")
                ...
                if math_interpretation:
                    description_parts.append(f"    Physics Interpretation: {math_interpretation}")
                
                # Meta-learning suitability
                if expr.complexity <= 10 and expr.r2_score > 0.7:
                    description_parts.append("    Meta-Learning Suitability: Excellent - simple and accurate")
                elif expr.complexity <= 20 and expr.r2_score > 0.5:
                    description_parts.append("    Meta-Learning Suitability: Good - reasonable complexity")
                else:
                    description_parts.append("    Meta-Learning Suitability: Limited - may overfit")
        ...
        description_parts.append(f"Overall Validation Score: {validation_score:.3f}/1.0")
        ...
        if validation_score >= 0.8:
            ...
            description_parts.append("  ✅ High confidence - Ready for meta-learning integration")
            description_parts.append("  ✅ Suitable for publication-quality experiments")
            description_parts.append("  ✅ Expected significant performance improvements")
        elif validation_score >= 0.6:
            description_parts.append("⚠️  RECOMMENDATION: PROCEED WITH CAUTION")
            description_parts.append("  ⚠️  Moderate confidence - Consider additional validation")
            ...
            description_parts.append("🔄 RECOMMENDATION: ADDITIONAL INVESTIGATION NEEDED")
            description_parts.append("  ❌ Low confidence - Requires further investigation")
            ...
            description_parts.append("  ❌ Very low confidence - Current approach insufficient")
            description_parts.append("  🔄 Recommend data quality assessment")
            description_parts.append("  🔄 Consider alternative physics discovery methods")
    ...
    def _get_physics_context(self, source: str, target: str) -> str:
        ...
        physics_contexts = {
            ('reynolds_number', 'viscosity'): "Reynolds number characterizes flow regime and affects apparent viscosity in turbulent flows",
            ('temperature', 'viscosity'): "Temperature dependence follows Arrhenius-type behavior in most fluids",
            ('shear_rate', 'viscosity'): "Shear-dependent viscosity indicates non-Newtonian fluid behavior",
            ('pressure', 'viscosity'): "Pressure effects on viscosity are typically small but measurable in high-pressure systems",
            ('velocity', 'viscosity'): "Velocity-dependent viscosity suggests complex flow-structure interactions"
        }
    ...
    def _interpret_mathematical_form(self, expr: sp.Expr) -> str:
        ...
        if 'exp' in expr_str:
            interpretations.append("Exponential relationship suggests activation energy or thermodynamic effects")
        
        if '**' in expr_str or 'pow' in expr_str:
            interpretations.append("Power law behavior indicates scaling relationships common in fluid dynamics")
        
        if 'log' in expr_str:
            interpretations.append("Logarithmic dependence suggests rate-limiting processes or boundary layer effects")
        
        if 'sin' in expr_str or 'cos' in expr_str:
            interpretations.append("Periodic behavior may indicate oscillatory flow phenomena or wave effects")
        
        if 'sqrt' in expr_str:
            interpretations.append("Square root dependence often appears in diffusion or boundary layer theory")
        
        # Check for linear combinations
        if '+' in expr_str and not any(op in expr_str for op in ['exp', '**', 'log', 'sin', 'cos']):
            interpretations.append("Linear combination suggests additive physical effects")
        
        return "; ".join(interpretations) if interpretations else "Complex mathematical relationship"
    
    def _generate_comprehensive_physics_interpretation(self,
                                                     causal_relationships: List[CausalRelationship],
                                                     symbolic_expressions: List[SymbolicExpression]) -> str:
        ...
        if causal_relationships and symbolic_expressions:
            ...
            symbolic_vars = {str(sym) for sym in symbolic_expressions[0].expression.free_symbols}
    ...
    def _generate_meta_learning_implications(self,
                                           causal_relationships: List[CausalRelationship],
                                           symbolic_expressions: List[SymbolicExpression],
                                           validation_score: float) -> str:
    ...
    def _generate_enhanced_meta_learning_implications(self,
                                                    causal_relationships: List[CausalRelationship],
                                                    symbolic_expressions: List[SymbolicExpression],
                                                    validation_score: float) -> str:
        """
        ...
        if causal_relationships:
            n_strong = len([r for r in causal_relationships if r.strength > 0.5])
            n_moderate = len([r for r in causal_relationships if 0.3 <= r.strength <= 0.5])
        ...
        strong_causal_count = len([r for r in causal_relationships if r.strength > 0.5]) if causal_relationships else 0
        high_quality_symbolic = len([e for e in symbolic_expressions if e.r2_score > 0.7]) if symbolic_expressions else 0
        ...
        if validation_score > 0.7:
            ...
            implications.append(
                "  2. Consider alternative physics discovery methods"
            )
            implications.append(
                "  3. Implement baseline meta-learning first, then gradually add physics constraints"
            )
        
        return "\n".join(implications) if implications else "No specific meta-learning implications identified"
    
    def _generate_physics_interpretation_integrated(self,
                                                  causal_relationships: List[CausalRelationship],
                                                  symbolic_expressions: List[SymbolicExpression]) -> str:
    ...
    def _meets_validation_criteria(self, hypothesis: PhysicsHypothesis) -> bool:
        ...
        return (
            hypothesis.validation_score >= self.validation_config['min_validation_score'] and
            hypothesis.meta_learning_improvement >= self.validation_config['min_improvement_threshold'] and
            hypothesis.confidence_score >= self.validation_config['confidence_threshold']
        )
    ...
    def export_discovery_results(self, 
        ...
        results_data = {
            ...
            'symbolic_analysis': {
                k: v for k, v in latest_result.symbolic_analysis.items()
    ...
    def _generate_discovery_plots(self, 
                                save_dir: Path, 
                                result: DiscoveryResult) -> Dict[str, str]:
        ...
        try:
            ...
            if result.hypothesis.causal_graph.number_of_nodes() > 0:
                ...
                self.causal_discovery.visualize_causal_graph(
                    result.hypothesis.causal_graph,
                    save_path=str(causal_plot_file)
                )
    ...
    def _compute_physics_consistency_score(self, hypothesis: PhysicsHypothesis) -> float:
        """
        ...
        if hypothesis.causal_relationships:
            strong_causal = [r for r in hypothesis.causal_relationships if r.strength > 0.5]
            causal_consistency = min(1.0, len(strong_causal) / 3)  # Normalize by expected max
            consistency_factors.append(causal_consistency * 0.4)
        
        # Symbolic expression consistency
        if hypothesis.symbolic_expressions:
            high_quality_symbolic = [e for e in hypothesis.symbolic_expressions if e.r2_score > 0.7]
            symbolic_consistency = min(1.0, len(high_quality_symbolic) / 2)  # Normalize by expected max
            consistency_factors.append(symbolic_consistency * 0.4)
        
        # Physics pattern consistency
        if hasattr(hypothesis, 'supporting_evidence') and 'physics_patterns' in hypothesis.supporting_evidence:
            n_patterns = len(hypothesis.supporting_evidence['physics_patterns'])
            pattern_consistency = min(1.0, n_patterns / 5)  # Normalize by expected max patterns
    ...
    def _generate_validation_insights(self, 
                                    validation_metrics: Dict[str, float],
                                    hypothesis: PhysicsHypothesis) -> str:
        """
        ...
        if overall_score >= 0.8:
            insights.append("🎯 Excellent validation: Discovered physics shows strong potential for meta-learning enhancement.")
        elif overall_score >= 0.6:
            insights.append("✅ Good validation: Physics discoveries should provide meaningful meta-learning improvements.")
        elif overall_score >= 0.4:
            insights.append("⚠️  Moderate validation: Some benefit expected but may be task-dependent.")
        else:
            insights.append("❌ Poor validation: Limited evidence for meta-learning improvement.")
        
        # Specific metric insights
        accuracy_improvement = validation_metrics.get('accuracy_improvement', 0)
        if accuracy_improvement > 0.1:
            insights.append(f"📈 Significant accuracy improvement expected: +{accuracy_improvement:.1%}")
        elif accuracy_improvement > 0.05:
            insights.append(f"📊 Moderate accuracy improvement: +{accuracy_improvement:.1%}")
        
        adaptation_speedup = validation_metrics.get('adaptation_speedup', 1.0)
        if adaptation_speedup > 1.5:
            insights.append(f"⚡ Fast adaptation: {adaptation_speedup:.1f}x speedup in convergence")
        elif adaptation_speedup > 1.2:
            insights.append(f"🚀 Improved adaptation: {adaptation_speedup:.1f}x faster convergence")
        
        # Physics quality insights
        if hypothesis.causal_relationships:
            strong_causal = len([r for r in hypothesis.causal_relationships if r.strength > 0.5])
            if strong_causal >= 2:
                insights.append(f"🔗 Strong causal structure: {strong_causal} robust relationships identified")
        
        if hypothesis.symbolic_expressions:
            best_r2 = max(e.r2_score for e in hypothesis.symbolic_expressions)
            if best_r2 > 0.8:
                insights.append(f"📐 High-quality mathematical relationships: R² = {best_r2:.3f}")
    ...
    def generate_comprehensive_physics_report(self, 
                                            discovery_result: DiscoveryResult,
        ...
        report_sections.append(f"Meta-Learning Improvement: {hypothesis.meta_learning_improvement:.1%}")
        report_sections.append(f"Causal Relationships Found: {len(hypothesis.causal_relationships)}")
        report_sections.append(f"Mathematical Expressions: {len(hypothesis.symbolic_expressions)}")
        ...
        # Validation analysis
        if include_validation and 'validation_insights' in discovery_result.validation_metrics:
            report_sections.append(f"\n✅ VALIDATION ANALYSIS")
            report_sections.append(discovery_result.validation_metrics['validation_insights'])
    ...
    def get_discovery_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'latest_validation_score': self.discovery_history[-1].hypothesis.validation_score,
            'latest_confidence_score': self.discovery_history[-1].hypothesis.confidence_score,
