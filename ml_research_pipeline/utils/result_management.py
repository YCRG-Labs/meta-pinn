    ...
    class ResultManager(LoggerMixin):
        ...
        def store_result(self, result: ExperimentResult):
            ...
            if result.training_history:
                result.convergence_analysis = self._analyze_convergence(result.training_history)
                result.statistical_summary = self._compute_statistical_summary(result.final_metrics)
        ...
        def compare_results(
            ...
            if comparison_id in self.comparisons:
                ...
                return ResultComparison(
                    ...
                    significance_matrix=np.array(cached_data['significance_matrix']) if cached_data.get('significance_matrix') else None,
            ...
            significance_matrix = self._create_significance_matrix(statistical_tests, experiment_ids)
        ...
        def _compare_metric_values(
            ...
            valid_ids = [exp_id for exp_id, v in zip(experiment_ids, values) if v is not None]
            ...
            comparison = {
                ...
                "best_experiment": valid_ids[np.argmin(valid_values)],  # Assuming lower is better
            ...
            for exp_id, value in zip(experiment_ids, values):
                if value is not None:
                    rel_diff = (value - best_value) / abs(best_value) if best_value != 0 else 0
            ...
            for metric in metrics:
                ...
                for i in range(len(metric_data)):
                    for j in range(i + 1, len(metric_data)):
                        ...
                        effect_size = abs(val_1 - val_2) / max(abs(val_1), abs(val_2), 1e-10)
                        
                        pairwise_tests[f"{exp_id_1}_vs_{exp_id_2}"] = {
                            "values": [val_1, val_2],
                            "difference": val_1 - val_2,
                            "relative_difference": (val_1 - val_2) / abs(val_2) if val_2 != 0 else 0,
                            "effect_size": effect_size,
                            "better_experiment": exp_id_1 if val_1 < val_2 else exp_id_2  # Assuming lower is better
                        }
            ...
            for result in results:
                ...
                for metric in metrics:
                    if metric in result.final_metrics:
                        ...
                        all_values = [r.final_metrics.get(metric) for r in results if metric in r.final_metrics]
                        ...
                        if all_values:
                            ...
                            if max_val != min_val:
                                normalized = (result.final_metrics[metric] - min_val) / (max_val - min_val)
                            else:
                                normalized = 0.0
        ...
        def _generate_comparison_summary(
            ...
            summary = {
                ...
                "best_overall_experiment": performance_ranking[0][0] if performance_ranking else None,
                "metric_winners": {},
                    ...
                    for comparison, test_result in tests["pairwise"].items():
                        ...
                        if effect_size > 0.2:  # Medium effect size threshold
                            summary["significant_differences"].append({
                                ...
                                "effect_size": effect_size,
                                "better_experiment": test_result.get("better_experiment")
                            })
        ...
        def _analyze_convergence(self, training_history: List[Dict[str, Any]]) -> Dict[str, Any]:
            ...
            analysis = {
                ...
                "relative_improvement": (loss_values[0] - loss_values[-1]) / abs(loss_values[0]) if loss_values[0] != 0 else 0
            }
            ...
            if window_size > 1:
                for i in range(window_size, len(loss_values)):
                    ...
                    if abs(min_recent) > 1e-10 and abs(current_loss - min_recent) / abs(min_recent) < 0.01:
            ...
            if len(final_portion) > 1:
                analysis["final_stability"] = {
                    ...
                    "coefficient_of_variation": np.std(final_portion) / abs(np.mean(final_portion)) if np.mean(final_portion) != 0 else 0
                }
        ...
        def generate_results_report(
            ...
            if completed_results:
                ...
                for metric in all_metrics:
                    values = [r.final_metrics[metric] for r in completed_results if metric in r.final_metrics]
                    if values:
                        performance_stats[metric] = {
                            ...
                            "min": np.min(values),
                            "max": np.max(values),
                            "best_experiment": [r.experiment_id for r in completed_results 
                                            if r.final_metrics.get(metric) == np.min(values)][0]
                        }
            ...
            reproducible_count = sum(
                1 for r in results 
                if r.reproducibility_info.get("validation_performed", False) and 
                r.reproducibility_info.get("overall_status") == "PASSED"
            )
            ...
            if include_comparisons and len(completed_results) > 1:
                comparison = self.compare_results([r.experiment_id for r in completed_results])
