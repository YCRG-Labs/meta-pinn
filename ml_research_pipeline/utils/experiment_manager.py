...
from .logging_utils import LoggerMixin, get_logger
from .random_utils import generate_experiment_seed, get_random_state, set_random_seeds
...
class ExperimentManager(LoggerMixin):
    ...
    def _generate_experiment_id(self) -> str:
        """Generate unique experiment ID."""
        # Use timestamp + config hash + random component for uniqueness
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")[
            :19
        ]  # Include microseconds, truncate to 6 digits
    ...
    def _setup_reproducibility(self):
        ...
        experiment_seed = generate_experiment_seed(self.config.seed, self.experiment_id)

        set_random_seeds(experiment_seed, deterministic=self.config.deterministic)
    ...
    def load_checkpoint(
        ...
        if checkpoint_path is None:
            checkpoint_path = (
                self.experiment_dir / "checkpoints" / "latest_checkpoint.pth"
            )
        else:
    ...
    def get_experiment_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "files": {
                "config": str(
                    self.experiment_dir / "config" / "experiment_config.yaml"
                ),
...
class ExperimentRegistry(LoggerMixin):
    ...
    def list_experiments(
        self, status: Optional[str] = None, name_pattern: Optional[str] = None
    ) -> List[Dict[str, Any]]:
