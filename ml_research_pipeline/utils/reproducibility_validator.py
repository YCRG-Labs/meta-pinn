...
from .random_utils import get_random_state, set_random_seeds, set_random_state
...
class ReproducibilityValidator(LoggerMixin):
    ...
    def validate_deterministic_forward_pass(
        ...
        for run in range(num_runs):
            ...
            use_deterministic = (
                os.getenv("PYTORCH_DETERMINISTIC", "true").lower() == "true"
            )
        ...
        if all_identical:
            ...
            self.log_warning(
                f"✗ Forward pass is not deterministic (max diff: {max_diff:.2e})"
            )
    ...
    def validate_deterministic_training_step(
        ...
        loss_identical = all(
            abs(loss - losses[0]) < training_tolerance for loss in losses
        )
        ...
        for i in range(1, len(final_states)):
            for key in final_states[0].keys():
                diff = (
                    torch.abs(final_states[i][key] - final_states[0][key]).max().item()
                )
    ...
    def validate_random_state_consistency(
        self, seed: int = 42, num_operations: int = 100
    ) -> Dict[str, Any]:
        ...
        if all_identical:
            ...
            self.log_warning(
                f"✗ Random state inconsistency detected (max diff: {max_diff:.2e})"
            )
    ...
    def validate_cross_platform_reproducibility(
        ...
        if reference_output is None:
            ...
            if passed:
                ...
                self.log_warning(
                    f"✗ Cross-platform reproducibility failed (diff: {diff:.2e})"
                )
    ...
    def validate_gradient_determinism(
        ...
        for i in range(1, len(gradients)):
            for name in gradients[0].keys():
                if name in gradients[i]:
                    diff = (
                        torch.abs(gradients[i][name] - gradients[0][name]).max().item()
                    )
                    max_diff = max(max_diff, diff)
        ...
        if all_identical:
            ...
            self.log_warning(
                f"✗ Gradient computation is not deterministic (max diff: {max_diff:.2e})"
            )
    ...
    def generate_reproducibility_report(
        self, validation_results: Dict[str, Any], output_file: Path
    ):
        ...
        report = {
            ...
            "summary": {
                "overall_status": (
                    "PASSED" if validation_results["overall_passed"] else "FAILED"
                ),
    ...
    def _generate_recommendations(
        ...
        if not validation_results["overall_passed"]:
            recommendations.append(
                "Some reproducibility tests failed. Review individual test results."
            )
        ...
        if validation_results["overall_passed"]:
            recommendations.append(
                "All reproducibility tests passed. Experiment should be fully reproducible."
            )
