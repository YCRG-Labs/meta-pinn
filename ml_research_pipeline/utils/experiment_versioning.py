...
class ExperimentVersionManager(LoggerMixin):
    ...
    def _load_version_db(self) -> Dict[str, Any]:
        ...
        if self.version_db_file.exists():
            with open(self.version_db_file, "r") as f:
                return json.load(f)
        return {"experiments": {}, "version_chains": {}, "tags": {}}
    ...
    def register_experiment(
        ...
        if (
            ...
            and parent_experiment_id in self.version_db["experiments"]
        ):
            self.version_db["experiments"][parent_experiment_id]["children_ids"].append(
                experiment_id
            )
    ...
    def compare_experiments(
        self, experiment_id_1: str, experiment_id_2: str, use_cache: bool = True
        ...
        if use_cache and (
            cache_key in self.comparison_cache
            or reverse_cache_key in self.comparison_cache
        ):
            cached_result = self.comparison_cache.get(
                cache_key
            ) or self.comparison_cache.get(reverse_cache_key)
    ...
    def _determine_reproducibility_status(
        ...
        significant_config_diffs = {
            k: v
            for k, v in config_differences.items()
    ...
    def generate_experiment_report(self, experiment_id: str) -> Dict[str, Any]:
        ...
        similar_experiments = self.find_similar_experiments(
            experiment_id, similarity_threshold=0.7
        )
    ...
    def _analyze_reproducibility(self, experiment_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            repro_report_file = (
                Path(manager.experiment_dir) / "reproducibility_report.json"
            )
            ...
            if repro_report_file.exists():
                ...
                return {
                    ...
                    "overall_status": repro_data.get("summary", {}).get(
                        "overall_status"
                    ),
                    "passed_tests": repro_data.get("summary", {}).get(
                        "passed_tests", 0
                    ),
    ...
    def cleanup_old_comparisons(self, days_old: int = 30):
        ...
        for key, comparison in self.comparison_cache.items():
            comparison_date = datetime.fromisoformat(
