...
class ExperimentTracker(LoggerMixin):
    ...
    def log_metric(
        ...
        with self.lock:
            ...
            # Auto-save if needed
            if step % self.auto_save_interval == 0:
                self._save_metrics()
            
            # Auto-plot if enabled
            if self.enable_plotting and step % (self.auto_save_interval * 5) == 0:
                self._update_plots()
    ...
    def get_all_metrics_statistics(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {name: tracker.get_statistics() for name, tracker in self.metrics.items()}
    ...
    def _save_metrics(self):
        ...
        # Save individual metric entries
    ...
    def _update_plots(self):
        ...
        for name, tracker in self.metrics.items():
            ...
            if len(values) > 10:
                ...
                plt.plot(rolling_steps, rolling_mean, label=f'{name} (rolling mean)', linewidth=2)
