...
class PaperPlotGenerator:
    ...
    def create_method_comparison_plot(self,
                                    ...
                                    significance_data: Optional[Dict] = None) -> plt.Figure:
        """
        ...
        ax.set_ylabel(ylabel or metric_name.replace('_', ' ').title(), fontweight='bold')
        ax.set_title(title or f'{metric_name.replace("_", " ").title()} Comparison', 
    ...
    def create_convergence_plot(self,
        ...
        ax.set_title(title or 'Training Convergence Comparison', fontweight='bold', pad=20)
    ...
    def create_uncertainty_plot(self,
        ...
        ax.plot(x_data, y_mean, color=self.colors['primary'], 
               linewidth=2, label='Prediction', alpha=0.8)
        
        # Plot confidence intervals
        ax.fill_between(x_data, y_mean - 2*y_std, y_mean + 2*y_std,
                       color=self.colors['primary'], alpha=0.2, label='95% CI')
        ax.fill_between(x_data, y_mean - y_std, y_mean + y_std,
                       color=self.colors['primary'], alpha=0.3, label='68% CI')
        
        # Plot ground truth if available
        if ground_truth is not None:
            ax.plot(x_data, ground_truth, color=self.colors['success'],
                   linewidth=2, linestyle='--', label='Ground Truth', alpha=0.8)
    ...
    def create_scatter_plot(self,
                          ...
                          ylabel: str = 'Y',
                          save_path: Optional[Path] = None,
                          add_regression_line: bool = False) -> plt.Figure:
        """
        ...
        if labels is None:
            ...
            ax.scatter(x_data, y_data, color=color, alpha=0.7, s=50, edgecolors='black', linewidth=0.5)
        else:
            # Multiple groups
            unique_labels = list(set(labels))
            for i, label in enumerate(unique_labels):
                mask = np.array(labels) == label
                color = colors[i] if colors and i < len(colors) else list(self.method_colors.values())[i % len(self.method_colors)]
                ax.scatter(x_data[mask], y_data[mask], label=label, color=color, 
        ...
        if add_regression_line:
            ...
            p = np.poly1d(z)
            ax.plot(x_data, p(x_data), color=self.colors['success'], 
    ...
    def _add_significance_indicators(self,
                                   ax: plt.Axes,
                                   x_pos: np.ndarray,
                                   means: List[float],
                                   stds: List[float],
                                   significance_data: Dict) -> None:
        ...
        for i, (method, stats) in enumerate(significance_data.items()):
            if 'p_value' in stats:
                ...
                ax.text(x_pos[i], y_pos, sig_text, ha='center', va='bottom',
