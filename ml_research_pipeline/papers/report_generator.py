...
class ReportGenerator:
    ...
    def generate_comprehensive_report(self,
                                    ...
                                    format_type: str = 'markdown') -> Dict[str, Path]:
        """
        ...
        # Executive Summary
        sections['executive_summary'] = self._generate_executive_summary(experimental_results)
        
        # Method Comparison Analysis
        sections['method_comparison'] = self._generate_method_comparison_analysis(experimental_results)
        
        # Performance Analysis
        sections['performance_analysis'] = self._generate_performance_analysis(experimental_results)
        
        # Statistical Analysis
        sections['statistical_analysis'] = self._generate_statistical_analysis(experimental_results)
        
        # Detailed Results
        sections['detailed_results'] = self._generate_detailed_results(experimental_results)
        ...
        self._save_report_metadata(experimental_results, metadata_path, report_title, author)
    ...
    def _generate_executive_summary(self, results: Dict[str, Any]) -> str:
        ...
        best_method, best_value, best_std = self._find_best_method(results, primary_metric)
        
        # Calculate improvement over baseline
        baseline_method = self._identify_baseline_method(methods)
        improvement = self._calculate_improvement(results, best_method, baseline_method, primary_metric)
        ...
        if improvement is not None:
            improvement_text = self.templates['executive_summary']['improvement'].format(
    ...
    def _generate_performance_analysis(self, results: Dict[str, Any]) -> str:
        ...
        if 'convergence_data' in results:
            convergence_analysis = self._analyze_convergence(results['convergence_data'])
        ...
        if 'scalability_data' in results:
            scalability_analysis = self._analyze_scalability(results['scalability_data'])
            analysis_parts.append(scalability_analysis)
        
        return ' '.join(analysis_parts) if analysis_parts else "Performance analysis data not available."
    ...
    def _generate_statistical_analysis(self, results: Dict[str, Any]) -> str:
        ...
        if 'statistical_tests' in results:
            significance_analysis = self._analyze_statistical_significance(results['statistical_tests'])
        ...
        if 'confidence_intervals' in results:
            ci_analysis = self._analyze_confidence_intervals(results['confidence_intervals'])
            analysis_parts.append(ci_analysis)
        
        return ' '.join(analysis_parts) if analysis_parts else "Statistical analysis data not available."
    ...
    def _generate_detailed_results(self, results: Dict[str, Any]) -> str:
        ...
        if 'hyperparameters' in results:
            hyperparameter_analysis = self._analyze_hyperparameters(results['hyperparameters'])
            details_parts.append(hyperparameter_analysis)
        
        return '\n\n'.join(details_parts)
    
    def _generate_report_plots(self, results: Dict[str, Any], output_dir: Path) -> Dict[str, Path]:
    ...
    def _generate_report_tables(self, results: Dict[str, Any], output_dir: Path) -> Dict[str, Path]:
    ...
    def _combine_report_sections(self,
    ...
    def _save_markdown_report(self, report_content: Dict[str, Any], filepath: Path) -> None:
    ...
    def _save_latex_report(self, report_content: Dict[str, Any], filepath: Path) -> None:
    ...
    def _save_html_report(self, report_content: Dict[str, Any], filepath: Path) -> None:
        ...
        for section_name in report_content['sections'].keys():
            formatted_name = section_name.replace('_', ' ').title()
            lines.append(f"            <li><a href='#{section_name}'>{formatted_name}</a></li>")
        lines.append("        </ul>")
        lines.append("    </div>")
        lines.append("")
    ...
    def _find_best_method(self, results: Dict[str, Any], metric: str) -> Tuple[Optional[str], float, float]:
    ...
    def _generate_method_ranking(self, results: Dict[str, Any], metric: str) -> List[str]:
    ...
    def _analyze_convergence(self, convergence_data: Dict[str, List[float]]) -> str:
        ...
        if fastest_method:
            return self.templates['performance_analysis']['convergence'].format(
                ...
                convergence_steps=fastest_steps
            )
        else:
    ...
    def _analyze_statistical_significance(self, statistical_tests: Dict[str, Any]) -> str:
        """Analyze statistical significance results."""
        significant_count = self._count_significant_results({'statistical_tests': statistical_tests})
        total_tests = sum(len(method_data) for method_data in statistical_tests.values())
    ...
    def _analyze_confidence_intervals(self, confidence_intervals: Dict[str, Any]) -> str:
    ...
    def _generate_method_details(self, method_name: str, method_data: Dict[str, Any]) -> str:
        ...
        for key, value in method_data.items():
            if key.endswith('_mean'):
                ...
                performance_metrics.append(f"{metric_name}: {value:.3f} ± {std_value:.3f}")
    ...
    def _analyze_physics_discoveries(self, physics_discovery_data: Dict[str, Any]) -> str:
        ...
        if 'discovered_relationships' in physics_discovery_data:
            ...
            if relationships:
                analysis_parts.append(f"The system discovered {len(relationships)} significant physical relationships:")
                ...
                analysis_parts.append("No significant physical relationships were discovered.")
        ...
        if 'causal_strengths' in physics_discovery_data:
            ...
            if causal_data:
                # Sort by strength
                sorted_causes = sorted(causal_data.items(), key=lambda x: x[1], reverse=True)
                
                analysis_parts.append("Variables ranked by causal influence on viscosity:")
                for variable, strength in sorted_causes:
                    strength_desc = "strong" if strength > 0.7 else "moderate" if strength > 0.4 else "weak"
                    analysis_parts.append(f"- **{variable}**: {strength:.2f} ({strength_desc} influence)")
        
        return "\n".join(analysis_parts) if analysis_parts else "Physics discovery analysis not available."