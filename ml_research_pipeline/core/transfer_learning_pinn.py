...
class TransferLearningPINN(StandardPINN):
    ...
    def pretrain(self, tasks: List[Dict[str, Any]], epochs_per_task: int = 500, 
        ...
        for task_idx, task in enumerate(tasks):
            ...
            if verbose:
                print(f"Pre-training on task {task_idx + 1}/{len(tasks)}: "
        ...
        if verbose:
            avg_final_loss = np.mean([task['final_loss'] for task in history['task_losses']])
            print(f"Pre-training completed. Average final loss: {avg_final_loss:.6f}")
        
        return history
    
    def finetune(self, task: Dict[str, Any], epochs: int = 100, 
                learning_rate: float = 0.0001, freeze_layers: Optional[List[int]] = None,
        ...
        if not self.is_pretrained:
            raise ValueError("Model must be pre-trained before fine-tuning. Call pretrain() first.")
    ...
    def pretrain_then_finetune(self, pretrain_tasks: List[Dict[str, Any]], 
    ...
    def evaluate_transfer_performance(self, test_tasks: List[Dict[str, Any]], 
                                    finetune_epochs: int = 50,
                                    finetune_lr: float = 0.0001) -> Dict[str, Any]:
        """
        ...
        for task_idx, task in enumerate(test_tasks):
            ...
            history = self.finetune(task, epochs=finetune_epochs, 
                                  learning_rate=finetune_lr, verbose=False)
        ...
        results['avg_final_loss'] = np.mean([r['final_loss'] for r in results['task_results']])
        results['avg_improvement'] = np.mean([r['improvement'] for r in results['task_results']])
        results['avg_convergence_speed'] = np.mean(results['convergence_speeds'])
        results['avg_transfer_effectiveness'] = np.mean(results['transfer_effectiveness'])
    ...
    def compare_with_scratch_training(self, task: Dict[str, Any], 
        ...
        pretrained_state = {name: param.clone() for name, param in self.named_parameters()}
        
        # 1. Fine-tuning approach
        finetune_history = self.finetune(task, epochs=finetune_epochs, 
        ...
        scratch_history = self.train_on_task(task, epochs=scratch_epochs, 
        ...
        comparison = {
            ...
            'transfer_advantage': scratch_history['total_loss'][-1] - finetune_history['total_loss'][-1],
            'sample_efficiency': scratch_epochs / finetune_epochs,
            'performance_ratio': finetune_history['total_loss'][-1] / scratch_history['total_loss'][-1]
        }
