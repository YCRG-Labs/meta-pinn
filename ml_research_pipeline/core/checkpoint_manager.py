...
class CheckpointManager:
    ...
    def save_checkpoint(
        ...
        """
        # Check if we should save (only main process in distributed training)
        ...
        try:
            ...
            if self.save_best and self.metric_name in metrics:
                ...
                if self._is_better_metric(metric_value):
                    ...
                    logging.info(
                        f"New best model saved: {best_path} ({self.metric_name}={metric_value:.6f})"
                    )
    ...
    def _update_metadata(
        ...
        if is_best or (
            ...
            and self._is_better_metric(metrics[self.metric_name])
        ):
    ...
    def _load_metadata(self) -> Dict[str, Any]:
        ...
        return {"checkpoints": [], "last_checkpoint": None, "best_checkpoint": None}
...
class AutoCheckpointer:
    ...
    def step(
        ...
        if (
            ...
            and step - self.last_save_step >= self.save_every_n_steps
        ):
            should_save = True
        ...
        if (
            ...
            and epoch - self.last_save_epoch >= self.save_every_n_epochs
        ):
            should_save = True
...
def create_checkpoint_manager(
    checkpoint_dir: Union[str, Path], config: Optional[MetaPINNConfig] = None, **kwargs
) -> CheckpointManager:
    """
