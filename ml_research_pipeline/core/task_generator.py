...
from ..config.data_config import DataConfig, GeometryConfig, TaskConfig, ViscosityConfig
...
class FluidTask:
        ...
        for dataset_name, dataset in [
            ...
            for key, tensor in dataset.items():
                if tensor.shape[0] != n_points:
                    raise ValueError(
                        f"Inconsistent number of points in {dataset_name}.{key}"
                    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "ground_truth": (
                {k: v.numpy() for k, v in self.ground_truth.items()}
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "FluidTask":
        ...
        ground_truth = (
            {k: torch.from_numpy(v) for k, v in data["ground_truth"].items()}
...
class FluidTaskGenerator:
    ...
    def __init__(
        ...
        if data_config.task_weights is None:
            self.task_weights = np.ones(len(data_config.task_types)) / len(
                data_config.task_types
            )
        else:
        ...
        logger.info(
            f"Initialized FluidTaskGenerator with {len(data_config.task_types)} task types"
        )
        ...
        for _ in range(batch_size):
            ...
            if task_types is None:
                task_type = np.random.choice(
                    self.data_config.task_types, p=self.task_weights
                )
            else:
    ...
    def _generate_viscosity_params(self, task_type: str) -> Dict[str, float]:
        ...
        params = {"base_viscosity": np.random.uniform(base_min, base_max)}
    ...
    def _sample_coordinates(
        self, n_points: int, task_config: TaskConfig
    ) -> torch.Tensor:
        ...
        if self.data_config.interior_sampling == "uniform":
            ...
            raise ValueError(
                f"Unknown sampling strategy: {self.data_config.interior_sampling}"
            )
    ...
    def _adaptive_sample(
        self, bounds: Tuple[float, float], n_points: int
    ) -> np.ndarray:
        """Adaptive sampling with higher density near boundaries."""
        ...
        for _ in range(n_boundary):
            if np.random.random() < 0.5:
                # Near lower boundary
                sample = bounds[0] + np.random.exponential(0.1) * (
                    bounds[1] - bounds[0]
                )
                sample = min(sample, bounds[1])
            else:
                # Near upper boundary
                sample = bounds[1] - np.random.exponential(0.1) * (
                    bounds[1] - bounds[0]
                )
    ...
    def validate_task_config(self, config: TaskConfig) -> bool:
        ...
        try:
            ...
            if not (10.0 <= config.reynolds_number <= 5000.0):
                logger.warning(
                    f"Reynolds number {config.reynolds_number} outside valid range"
                )
                return False
