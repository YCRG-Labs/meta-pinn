...
from ..utils.distributed_utils import get_rank, get_world_size, is_main_process, reduce_dict
...
class MetricTracker:
    ...
    def update(self, metrics: Dict[str, Union[float, torch.Tensor]]):
        ...
        for name, value in metrics.items():
            ...
            self.global_metrics[name].append({
                'value': value,
                'timestamp': timestamp
            })
    ...
    def get_history(self, metric_name: str, last_n: Optional[int] = None) -> List[Dict[str, Any]]:
        """
...
class TrainingMonitor:
    ...
    def __init__(
        ...
        self.log_dir = Path(log_dir)
        self.tensorboard_dir = Path(tensorboard_dir) if tensorboard_dir else self.log_dir / "tensorboard"
