...
class DistributedMetaPINN(nn.Module):
    ...
    def _setup_distributed_model(
        self, device_ids: Optional[List[int]], find_unused_parameters: bool
    ):
    ...
    def adapt_to_task(
        self, task: Dict[str, Any], adaptation_steps: Optional[int] = None
    ) -> OrderedDict:
        """
    ...
    def _distribute_tasks(
        self, task_batch: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
    ...
    def _compute_local_meta_gradients(
        ...
        return {"meta_loss": total_meta_loss / num_tasks, "num_tasks": num_tasks}

    def _compute_query_loss(
        self, task: Dict[str, Any], adapted_params: OrderedDict
    ) -> torch.Tensor:
        ...
        # Combined loss
        total_loss = (
            data_loss + self.config.physics_loss_weight * physics_losses["total"]
        )
    ...
    def _aggregate_metrics(self, local_metrics: Dict[str, float]) -> Dict[str, float]:
        ...
        metrics_tensor = torch.tensor(
            [local_metrics["meta_loss"], local_metrics["num_tasks"]],
        ...
        return {"meta_loss": total_loss / max(total_tasks, 1), "num_tasks": total_tasks}
    ...
    def load_checkpoint(self, filepath: str) -> Dict[str, Any]:
        ...
        checkpoint = torch.load(
            filepath, map_location=self.device if hasattr(self, "device") else "cpu"
        )
