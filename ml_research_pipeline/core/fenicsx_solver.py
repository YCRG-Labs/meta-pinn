...
try:
    ...
    logger.warning("FEniCSx not available. High-fidelity solver will not be functional.")
...
class FEniCSxSolver:
    ...
    def __init__(self, solver_config: Optional[SolverConfig] = None):
        ...
        if not FENICSX_AVAILABLE:
            raise ImportError("FEniCSx is required but not available. Please install FEniCSx.")
    ...
    def solve_task(self, 
                   ...
                   coordinates: torch.Tensor) -> AnalyticalSolution:
        """
        ...
        """
        logger.debug(f"Solving task {task_config.task_id} with {len(coordinates)} evaluation points")
        ...
        try:
            ...
            F = self._setup_variational_problem(task_config, u, p, v, q, viscosity_expr, domain)
    ...
    def _setup_function_spaces(self, domain):
        ...
        P2 = ufl.VectorElement("Lagrange", domain.ufl_cell(), self.solver_config.element_degree)
        P1 = ufl.FiniteElement("Lagrange", domain.ufl_cell(), self.solver_config.pressure_degree)
            ...
            if inlet_bc['type'] == 'dirichlet':
                ...
                try:
                    ...
                    if len(inlet_dofs) > 0:
                        ...
                        if geometry_type == 'channel':
                            inlet_profile = task_config.geometry_params.get('inlet_profile', 'parabolic')
                            
                            if inlet_profile == 'parabolic':
                                # Parabolic inlet profile: u(y) = 4*u_max*y*(H-y)/H^2
                                width = task_config.geometry_params.get('width', 1.0)
                                u_max = inlet_value[0] if isinstance(inlet_value, (list, tuple)) else inlet_value
                                ...
                                def inlet_expr(x):
                                    ...
                                    u_inlet = u_max * (
                                        np.tanh((y - transition_width) / (0.1 * transition_width)) - 
                                        np.tanh((y - width + transition_width) / (0.1 * transition_width))
                                    ) / 2
                                ...
                                def inlet_expr(x):
                                    u_inlet = np.full_like(x[0], inlet_value[0] if isinstance(inlet_value, (list, tuple)) else inlet_value)
                                    v_inlet = np.full_like(x[0], inlet_value[1] if isinstance(inlet_value, (list, tuple)) and len(inlet_value) > 1 else 0.0)
                ...
                logger.warning("Neumann inlet boundary conditions not fully implemented")
        ...
        if 'cylinder' in boundary_conditions:
            ...
            if cylinder_bc['type'] == 'dirichlet':
                ...
                def cylinder_boundary(x):
                    ...
                    dist = np.sqrt((x[0] - cylinder_pos[0])**2 + (x[1] - cylinder_pos[1])**2)
                    return dist <= cylinder_radius * 1.1  # Slightly larger for mesh points
                
                try:
                    cylinder_dofs = fem.locate_dofs_geometrical(W.sub(0), cylinder_boundary)
                    if len(cylinder_dofs) > 0:
                        cylinder_velocity = np.array(cylinder_value, dtype=PETSc.ScalarType)
                        bc_cylinder = fem.dirichletbc(cylinder_velocity, cylinder_dofs, W.sub(0))
                        bcs.append(bc_cylinder)
                except:
                    # Skip cylinder BC if no points found (mesh too coarse)
                    logger.warning("Could not apply cylinder boundary condition - mesh may be too coarse")
    ...
    def _create_viscosity_function(self, task_config: TaskConfig, domain):
        ...
        if task_type == 'linear_viscosity':
            ...
            viscosity_expr = mu_0 + grad_x * x[0] + grad_y * x[1] + cross_term * x[0] * x[1]
            ...
            viscosity_expr = mu_0 * amplitude * ufl.exp(decay_rate_x * x[0] + decay_rate_y * x[1])
            ...
            shear_rate_approx = ufl.sqrt(x[0]**2 + x[1]**2) + 1e-6  # Approximate shear rate
            ...
            if abs(flow_behavior_index - 1.0) < 1e-6:
                ...
                viscosity_expr = mu_0 + consistency_index * ufl.pow(shear_rate_approx, flow_behavior_index - 1.0)
            ...
            for key, coeff in coeffs.items():
                if '_' in key:  # Format: 'x2_y1' for x^2 * y^1
                    ...
                    viscosity_expr += coeff * ufl.pow(x[0], x_power) * ufl.pow(x[1], y_power)
            ...
            viscosity_expr = mu_0 + amplitude * ufl.sin(freq_x * x[0] + freq_y * x[1] + phase)
    ...
    def _setup_variational_problem(self, task_config: TaskConfig, u, p, v, q, viscosity_expr, domain):
        ...
        if reynolds > 100:
            ...
            F = (1/reynolds) * ufl.inner(viscosity_expr * ufl.grad(u), ufl.grad(v)) * ufl.dx
    ...
    def _solve_problem(self, F, bcs, W):
        ...
        problem = fem.petsc.LinearProblem(
            a, L, bcs=bcs, u=w,
            petsc_options={
                "ksp_type": "preonly" if self.solver_config.solver_type == "direct" else "gmres",
                "pc_type": "lu" if self.solver_config.solver_type == "direct" else "ilu",
    ...
    def _evaluate_solution(self, w, coordinates: torch.Tensor, task_config: TaskConfig) -> AnalyticalSolution:
        ...
        try:
            ...
            logger.warning(f"Failed to evaluate FEniCSx solution: {e}. Using fallback values.")
            # Fallback to zeros if evaluation fails
            velocity = torch.zeros(len(coordinates), 2)
            pressure = torch.zeros(len(coordinates), 1)
            viscosity_field = torch.full((len(coordinates), 1), 
    ...
    def _evaluate_viscosity_field(self, coordinates: torch.Tensor, task_config: TaskConfig) -> torch.Tensor:
    ...
    def validate_against_analytical(self, 
                                  ...
                                  analytical_solution: AnalyticalSolution) -> Dict[str, float]:
        """
        ...
        metrics = {
            ...
            'relative_velocity_error': (torch.mean(velocity_diff) / 
                                      torch.mean(torch.norm(analytical_solution.velocity, dim=1))).item(),
            'relative_pressure_error': (torch.mean(pressure_diff) / 
                                      torch.mean(torch.abs(analytical_solution.pressure))).item()
        }
    ...
    def _validate_task_config(self, task_config: TaskConfig):
        ...
        if not (0.1 <= task_config.reynolds_number <= 10000.0):
            raise ValueError(f"Reynolds number {task_config.reynolds_number} outside valid range [0.1, 10000]")
        
        # Check viscosity parameters
        base_visc = task_config.viscosity_params.get('base_viscosity', 0.01)
        if not (1e-6 <= base_visc <= 1e3):
            raise ValueError(f"Base viscosity {base_visc} outside physical bounds [1e-6, 1e3]")
    ...
    def _validate_solution_quality(self, solution: AnalyticalSolution, task_config: TaskConfig):
        """Validate solution quality and physics consistency."""
        # Check for NaN or infinite values
        if torch.any(torch.isnan(solution.velocity)) or torch.any(torch.isinf(solution.velocity)):
            raise ValueError("Solution contains NaN or infinite velocity values")
        
        if torch.any(torch.isnan(solution.pressure)) or torch.any(torch.isinf(solution.pressure)):
        ...
        if pressure_range.item() > 1e6:
            logger.warning(f"Pressure range {pressure_range.item()} seems unreasonably large")
    
    def _create_fallback_solution(self, coordinates: torch.Tensor, task_config: TaskConfig, error_msg: str) -> AnalyticalSolution:
    ...
    def solve_task_batch(self, 
                        task_configs: List[TaskConfig], 
                        coordinates: torch.Tensor) -> List[AnalyticalSolution]:
        """
        ...
        for i, task_config in enumerate(task_configs):
            logger.debug(f"Solving task {i+1}/{len(task_configs)}: {task_config.task_id}")
            ...
            try:
                ...
                logger.error(f"Failed to solve task {task_config.task_id}: {e}")
                fallback_solution = self._create_fallback_solution(coordinates, task_config, str(e))
    ...
    def generate_ground_truth_dataset(self, 
        ...
        for i, task_config in enumerate(task_configs):
            ...
            coordinates = self._generate_evaluation_coordinates(task_config, n_points_per_task)
            ...
            task_data = {
                ...
                'viscosity_field': solution.viscosity_field.numpy() if solution.viscosity_field is not None else None,
        ...
        logger.info(f"Generated ground truth dataset with {len(dataset['tasks'])} tasks")
        return dataset
    
    def _generate_evaluation_coordinates(self, task_config: TaskConfig, n_points: int) -> torch.Tensor:
        ...
        x_interior = np.random.uniform(x_bounds[0] + 0.01, x_bounds[1] - 0.01, n_interior)
        y_interior = np.random.uniform(y_bounds[0] + 0.01, y_bounds[1] - 0.01, n_interior)
        ...
        y_right = np.random.uniform(y_bounds[0], y_bounds[1], n_boundary - 3 * n_per_boundary)
    ...
    def _convert_dataset_for_json(self, dataset: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for task_data in dataset['tasks']:
            json_task = {
                ...
                'viscosity_field': task_data['viscosity_field'].tolist() if task_data['viscosity_field'] is not None else None,
...
def create_fenicsx_solver(solver_config: Optional[SolverConfig] = None) -> Optional[FEniCSxSolver]:
    """
