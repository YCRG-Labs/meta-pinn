...
class EvaluationMetrics:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Initialized EvaluationMetrics with {len(self.metric_registry)} metrics"
        )
        ...
        for metric_name, metric_func in self.metric_registry.items():
            try:
                ...
                all_metrics[metric_name] = MetricResult(
                    name=metric_name, value=float("nan"), metadata={"error": str(e)}
                )

        return all_metrics

    def _compute_parameter_accuracy(
        self,
        results: Dict[str, List[Dict[str, Any]]],
        ground_truth: Optional[Dict[str, Any]] = None,
    ) -> MetricResult:
        ...
        for method_name, method_results in results.items():
            for task_result in method_results:
                if "computation_time" in task_result:
                    ...
                    if (
                        not np.isnan(time_val)
                        and not np.isinf(time_val)
                        and time_val > 0
                    ):
    ...
    def _compute_convergence_rate(
        ...
        for method_name, method_results in results.items():
            for task_result in method_results:
                ...
                if (
                    "adaptation_steps" in task_result
                    and "parameter_accuracy" in task_result
                ):
...
class StatisticalAnalysis:
    ...
    def compare_methods(
        self,
        results: Dict[str, List[Dict[str, Any]]],
        metric_name: str = "parameter_accuracy",
    ...
    def _perform_statistical_test(
        ...
        interpretation = self._interpret_test_result(
            p_value, effect_size, method1, method2
        )
    ...
    def _interpret_test_result(
        self, p_value: float, effect_size: float, method1: str, method2: str
    ) -> str:
    ...
    def _compute_difference_ci(
        self, values1: List[float], values2: List[float], confidence: float = 0.95
    ) -> Tuple[float, float]:
        """Compute confidence interval for difference in means."""
        mean1, mean2 = np.mean(values1), np.mean(values2)
        std1, std2 = np.std(values1, ddof=1), np.std(values2, ddof=1)
    ...
    def perform_power_analysis(
        self, effect_size: float, alpha: float = None, power: float = 0.8
    ) -> Dict[str, float]:
        """
