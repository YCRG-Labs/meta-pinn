...
from .benchmark_suite import PINNBenchmarkSuite, BenchmarkResult
from .metrics import EvaluationMetrics, StatisticalAnalysis, MetricResult, StatisticalTest
...
class MethodComparison:
    ...
    def __init__(self, config: MethodComparisonConfig):
        ...
        logger.info(f"Initialized MethodComparison with {len(config.benchmarks)} benchmarks")
    ...
    def _run_benchmarks(self, methods: Dict[str, Any]) -> Dict[str, Dict[str, BenchmarkResult]]:
        ...
        for benchmark_name in self.config.benchmarks:
            ...
            for method_name, method in methods.items():
                try:
                    ...
                    logger.error(f"Error running {method_name} on {benchmark_name}: {e}")
    ...
    def _compute_metrics(self, 
                        benchmark_results: Dict[str, Dict[str, BenchmarkResult]]) -> Dict[str, Dict[str, MetricResult]]:
        ...
        for benchmark_name, benchmark_data in benchmark_results.items():
            ...
            # Compute all metrics
            benchmark_metrics = self.metrics_calculator.compute_all_metrics(results_for_metrics)
            metric_results[benchmark_name] = benchmark_metrics
        
        return metric_results
    
    def _perform_statistical_analysis(self, 
                                    benchmark_results: Dict[str, Dict[str, BenchmarkResult]]) -> Dict[str, Dict[str, Dict[str, StatisticalTest]]]:
        ...
        for benchmark_name, benchmark_data in benchmark_results.items():
            ...
            for metric_name in self.config.metrics:
                try:
                    ...
                    logger.warning(f"Failed statistical analysis for {metric_name}: {e}")
    ...
    def _generate_rankings(self, 
                          metric_results: Dict[str, Dict[str, MetricResult]]) -> Dict[str, List[Tuple[str, float]]]:
        ...
        for benchmark_name, benchmark_metrics in metric_results.items():
            for metric_name, metric_result in benchmark_metrics.items():
                if not np.isnan(metric_result.value) and not np.isinf(metric_result.value):
    ...
    def _generate_summary(self, 
        ...
        for benchmark_name, benchmark_data in benchmark_results.items():
            ...
            n_tasks = sum(len(result.task_results) for result in benchmark_data.values())
            total_time = sum(result.runtime_info.get('total_time', 0) for result in benchmark_data.values())
            
            summary['benchmark_summary'][benchmark_name] = {
                'n_methods': n_methods,
                'n_tasks': n_tasks,
                'total_runtime': total_time,
                'avg_runtime_per_method': total_time / n_methods if n_methods > 0 else 0
            }
        ...
        for method_name in method_names:
            ...
            for benchmark_name, benchmark_data in benchmark_results.items():
                if (method_name in benchmark_data and 
                    'error' not in benchmark_data[method_name].metadata):
    ...
    def _save_results(self, comparison_result: ComparisonResult):
        ...
        for benchmark_name, benchmark_data in comparison_result.benchmark_results.items():
            serializable_results['benchmark_results'][benchmark_name] = {}
            for method_name, result in benchmark_data.items():
                serializable_results['benchmark_results'][benchmark_name][method_name] = {
        ...
        for benchmark_name, benchmark_metrics in comparison_result.metric_results.items():
        ...
        for benchmark_name, benchmark_stats in comparison_result.statistical_results.items():
            serializable_results['statistical_results'][benchmark_name] = {}
            for metric_name, metric_comparisons in benchmark_stats.items():
                serializable_results['statistical_results'][benchmark_name][metric_name] = {}
                for method1, method1_comparisons in metric_comparisons.items():
                    serializable_results['statistical_results'][benchmark_name][metric_name][method1] = {}
                    for method2, test_result in method1_comparisons.items():
                        serializable_results['statistical_results'][benchmark_name][metric_name][method1][method2] = {
    ...
    def _generate_tables(self, comparison_result: ComparisonResult):
        ...
        for benchmark_name in comparison_result.config.benchmarks:
            if benchmark_name in comparison_result.benchmark_results:
                ...
                table_file = self.save_dir / f"{benchmark_name}_comparison_{timestamp_str}.tex"
    ...
    def _create_summary_table(self, comparison_result: ComparisonResult) -> str:
        ...
        for benchmark_name, benchmark_summary in comparison_result.summary['benchmark_summary'].items():
    ...
    def _create_comparison_table(self, comparison_result: ComparisonResult, 
        ...
        for method_name in method_names:
            ...
            for metric_name in self.config.metrics:
                if (benchmark_name in comparison_result.metric_results and
                    metric_name in comparison_result.metric_results[benchmark_name]):
                    
                    metric_result = comparison_result.metric_results[benchmark_name][metric_name]
                    value = metric_result.value
    ...
    def load_comparison_results(self, results_file: str) -> ComparisonResult:
        ...
        config = MethodComparisonConfig(
            ...
            multiple_comparison_correction=data['config']['multiple_comparison_correction']
        )
    ...
    def get_method_ranking(self, comparison_result: ComparisonResult, 
                          metric_name: str) -> List[Tuple[str, float]]:
