    ...
    def optimize_memory(self) -> Dict[str, Any]:
        ...
        if current_usage.get("cpu_memory_percent", 0) > 80:
            recommendations.append(
                "High CPU memory usage - consider reducing batch size"
            )

        if current_usage.get("gpu_memory_percent", 0) > 80:
            recommendations.append(
                "High GPU memory usage - consider gradient checkpointing"
            )

        if self.peak_gpu_memory > current_usage.get("gpu_memory_allocated_gb", 0) * 2:
            recommendations.append(
                "Large memory spikes detected - consider memory profiling"
            )
...
class ScalabilityTester:
    ...
    def test_batch_size_scaling(
        ...
        for batch_size in batch_sizes:
            ...
            try:
                ...
                results.append({"batch_size": batch_size, "error": str(e)})
        ...
        for i, config in enumerate(model_configs):
            ...
            try:
                ...
                result = {
                    ...
                    "params_per_second": (
                        total_params / metrics.cpu_time if metrics.cpu_time > 0 else 0
                    ),
                }
    ...
    def test_distributed_scaling(
        ...
        for world_size in world_sizes:
            ...
            try:
                ...
                results.append({"world_size": world_size, "error": str(e)})
    ...
    def _generate_scaling_recommendations(self) -> List[str]:
        ...
        recommendations.append(
            "Use mixed precision training to improve memory efficiency"
        )
...
def benchmark_function(
    func: Callable, *args, num_runs: int = 10, warmup_runs: int = 3, **kwargs
) -> PerformanceMetrics:
    """
...
class PerformanceBenchmarkSuite:
    ...
    def run_full_benchmark(
        self, model_fn: Callable, test_configs: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if "batch_sizes" in test_configs:
                ...
                results["batch_size_scaling"] = (
                    self.scalability_tester.test_batch_size_scaling(
                        model_fn=model_fn,
            ...
            if "model_configs" in test_configs:
                ...
                results["model_size_scaling"] = (
                    self.scalability_tester.test_model_size_scaling(
            ...
            if "world_sizes" in test_configs:
                ...
                results["distributed_scaling"] = (
                    self.scalability_tester.test_distributed_scaling(
                        model_fn=model_fn,
            ...
            results["optimization_report"] = (
                self.profiler.generate_optimization_report()
            )
            results["scalability_report"] = (
                self.scalability_tester.generate_scalability_report()
            )
            ...
            info.update(
                {
                    ...
                    "cuda_memory_gb": torch.cuda.get_device_properties(0).total_memory
                    / 1024**3,
                }
            )
