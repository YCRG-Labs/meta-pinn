...
class PerformanceRegressionTester:
    ...
    def run_regression_test(
        ...
        **kwargs
        ...
        if test_name not in self.baselines:
            ...
            baseline = self.create_baseline(test_name, test_function, test_config, *args, **kwargs)
        ...
        if baseline.config_hash != current_config_hash:
            logging.warning(f"Configuration changed for {test_name}, results may not be comparable")
    ...
    def _compare_with_baseline(
        ...
        overall_regression = max(regression_percentages.values()) if regression_percentages else 0.0
        
        details = {
            'regression_percentages': regression_percentages,
            'baseline_timestamp': self.baselines[test_name].timestamp if test_name in self.baselines else None,
    ...
    def _update_baseline(
        ...
        if test_name in self.baselines:
            ...
            self.baselines[test_name].config_hash = self._compute_config_hash(test_config)
        ...
        for test_name, test_spec in test_suite.items():
            try:
                ...
                if result.regression_detected:
                    logging.warning(f"Regression detected in {test_name}: "
    ...
    def generate_regression_report(
        self,
        results: Optional[Dict[str, RegressionResult]] = None
    ) -> Dict[str, Any]:
        """
        ...
        regressions_detected = sum(1 for r in valid_results.values() if r.regression_detected)
        ...
        report = {
            ...
            'summary': {
                ...
                'regression_rate': (regressions_detected / total_tests * 100) if total_tests > 0 else 0,
            ...
            'detailed_results': {name: result.to_dict() for name, result in valid_results.items()},
            'regression_categories': {
                'severe': [{'test': name, 'regression_pct': result.regression_percentage} 
                          for name, result in severe_regressions],
                'moderate': [{'test': name, 'regression_pct': result.regression_percentage} 
                            for name, result in moderate_regressions],
                'minor': [{'test': name, 'regression_pct': result.regression_percentage} 
    ...
    def _generate_regression_recommendations(
        ...
        memory_regressions = sum(1 for r in results.values() 
                               if r.threshold_exceeded.get('memory_peak', False))
        cpu_regressions = sum(1 for r in results.values() 
                            if r.threshold_exceeded.get('cpu_time', False))
        gpu_regressions = sum(1 for r in results.values() 
                            if r.threshold_exceeded.get('gpu_time', False))
        ...
        if len([r for r in results.values() if r.regression_detected]) > len(results) * 0.3:
            recommendations.append(
