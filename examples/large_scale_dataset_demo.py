...
def main():
    ...
    try:
        ...
        for split_name, split_info in dataset_info["splits"].items():
            print(
                f"  {split_name}: {split_info['n_tasks']} tasks "
        ...
        print(
            f"  Average task generation time: {perf_stats['average_task_time']*1000:.2f} ms"
        )
        ...
        print(
            f"  Reynolds number range: {stats['reynolds_statistics']['min']:.1f} - "
            f"{stats['reynolds_statistics']['max']:.1f}"
        )
        print(
            f"  Average support points: {stats['data_point_statistics']['support_points']['mean']:.0f}"
        )
        print(
            f"  Average query points: {stats['data_point_statistics']['query_points']['mean']:.0f}"
        )
        ...
        loader = DatasetLoader(
            dataset_path=dataset_info["save_path"], cache_size=200, prefetch_size=100
        )
        ...
        for i, batch in enumerate(
            loader.iterate_batches("train", batch_size, shuffle=True)
        ):
        ...
        for i, batch in enumerate(
            loader.iterate_batches_streaming("train", batch_size, shuffle=False)
        ):
        ...
        # Validate dataset integrity (skip checksum for random data)
        print("\nValidating dataset integrity...")
        validation_report = loader.validate_dataset_integrity()

        # Filter out checksum errors since we're using random data
        non_checksum_errors = [
        ...
        if not non_checksum_errors:
            ...
            for error in non_checksum_errors:
                print(f"  - {error}")
