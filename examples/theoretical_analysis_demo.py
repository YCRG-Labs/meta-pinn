...
from theory.sample_complexity import ComplexityParameters, SampleComplexityAnalyzer
...
def demonstrate_sample_complexity_analysis():
    ...
    for n_tasks in task_counts:
        ...
        print(
            f"Tasks: {n_tasks:3d} | Improvement: {bounds.improvement_factor:.2f}x | "
...
def demonstrate_convergence_analysis():
    ...
    print(
        f"Adaptation bound:       {analysis.error_bounds['task_adaptation_bound']:.4f}"
    )
    print(
        f"Generalization bound:   {analysis.error_bounds['meta_generalization_bound']:.4f}"
    )
    print(
        f"Physics improvement:    {analysis.error_bounds['physics_improvement']:.2f}x"
    )
...
def demonstrate_formal_proofs():
    ...
    if os.path.exists(output_path):
        ...
        print(
            f"Contains: {theorem_count} theorems, {proof_count} proofs, {equation_count} equations"
        )
...
def demonstrate_integration():
    ...
    print(
        f"  Physics benefit: {convergence_analysis.error_bounds['physics_improvement']:.2f}x"
    )

    # Theoretical consistency check
    print(f"\nConsistency Check:")
    physics_benefit_sample = sample_bounds.improvement_factor
    physics_benefit_convergence = convergence_analysis.error_bounds[
        "physics_improvement"
    ]
