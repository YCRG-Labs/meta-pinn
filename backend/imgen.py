...
def load_existing_model(model_path):
    ...
    if not CONFIG_AVAILABLE:
        print("❌ Config not available, cannot load model")
        return None
        ...
        for item in os.listdir(results_dir):
            ...
            if os.path.isdir(item_path) and item.startswith("inference_"):
                ...
                if has_files:
                    ...
                    print(f"✅ Found interactive.py data: {item}")
        ...
        for item in os.listdir(examples_dir):
            ...
            if os.path.isdir(item_path) and item.startswith("scenario_"):
                ...
                if os.path.exists(inference_subdir):
                    ...
                    print(f"✅ Found app.py scenario data: {item}")
    ...
    for source_name, source_info in data_sources.items():
        ...
        if os.path.exists(visc_file):
            ...
            if viscosity_data is not None:
                data_source_name = source_name
                print(f"📊 Using viscosity data from: {source_name}")
                break
    
    if viscosity_data is None:
        print("❌ No viscosity profile data found")
        return None
...
Parameter Error:
• Absolute:    {abs_param_error:.6f}
• Relative:    {rel_param_error:.2f}%
...
"""
    ...
    print(f"✅ Viscosity analysis saved: {save_path}")
    return save_path
...
def generate_3d_visualization(model, save_dir):
    ...
    if model is None or not CONFIG_AVAILABLE:
        print("⚠️  Cannot generate 3D visualizations: model not available")
        return None
...
def generate_multi_scenario_comparison(data_sources, save_dir):
    ...
    if not scenario_data:
        print("⚠️  No app.py scenario data found for comparison")
        return None
    ...
    for scenario_id, scenario_info in scenario_data.items():
        ...
        if os.path.exists(metadata_file):
            try:
                ...
                print(f"⚠️  Could not load metadata for {scenario_id}: {e}")
    
    if not scenarios:
        print("❌ No valid scenario metadata found")
        return None
...
"""
    ...
    print(f"✅ Multi-scenario comparison saved: {save_path}")
    return save_path
...
def main():
    ...
    print("="*70)
    print("🎨 PINN Presentation Visualization Generator (FIXED)")
    ...
    if not data_sources:
        ...
        print("  • python app.py (and generate scenarios)")
